...
locals {
  # Certificate with extra whitespace (blank lines at start and end)
  certificate_dirty = base64encode(<<-EOT
...
EOT
  ...
  # Intermediates with comments and whitespace
  intermediates_dirty = base64encode(<<-EOT
...
resource "upcloud_loadbalancer_manual_certificate_bundle" "this" {
  ...
  certificate   = local.certificate_dirty
  intermediates = local.intermediates_dirty
