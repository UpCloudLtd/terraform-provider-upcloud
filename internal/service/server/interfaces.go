...
func interfacesToMap(ifaces []networkInterfaceModel) map[int]networkInterfaceModel {
	m := make(map[int]networkInterfaceModel)
...
func findInterfaceFromState(ifaces map[int]networkInterfaceModel, index int, ip, t string) (*networkInterfaceModel, map[int]networkInterfaceModel) {
	for i, iface := range ifaces {
		if int(iface.Index.ValueInt64()) == index {
			delete(ifaces, i)
			return &iface, ifaces
		}
	}
	for i, iface := range ifaces {
		if iface.IPAddress.ValueString() == ip && iface.Type.ValueString() == t {
			delete(ifaces, i)
			return &iface, ifaces
		}
	}
...
func canModifyInterface(plan networkInterfaceModel, prev *upcloud.ServerInterface) bool {
	if prev.Type != plan.Type.ValueString() {
		return false
	}
	if len(prev.IPAddresses) > 0 && prev.IPAddresses[0].Family != plan.IPAddressFamily.ValueString() {
		return false
	}
	if prev.Type == upcloud.NetworkTypePrivate {
		...
		if len(prev.IPAddresses) > 0 && prev.IPAddresses[0].Address != plan.IPAddress.ValueString() {
			return false
		}
	}
	return true
...
func updateServerNetworkInterfaces(ctx context.Context, svc *service.Service, data *serverModel) (diags diag.Diagnostics) {
	...
	diags.Append(data.NetworkInterfaces.ElementsAs(ctx, &networkInterfacesPlan, false)...)

	indicesToKeep := map[int]bool{}
	n := len(networkInterfacesPlan)

	newNetworkInterfaces := make([]networkInterfaceModel, n)
	networkInterfaces := interfacesToMap(networkInterfacesPlan)
	modifiedInterfaces := make(map[int]*upcloud.Interface)
	indices := make(map[int]bool)

	// Try to modify existing server interfaces
	var val *networkInterfaceModel
	for _, iface := range s.Networking.Interfaces {
		indices[iface.Index] = true
		val, networkInterfaces = findInterfaceFromState(networkInterfaces, iface.Index, iface.IPAddresses[0].Address, iface.Type)
		...
		if shouldModifyInterface(*val, addresses, &iface) {
			req := request.ModifyNetworkInterfaceRequest{
				...
				NewIndex:          int(val.Index.ValueInt64()),
	...
	data.NetworkInterfaces, d = types.ListValueFrom(ctx, data.NetworkInterfaces.ElementType(ctx), newNetworkInterfaces)
	diags.Append(d...)
	return
}
